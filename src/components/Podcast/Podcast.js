import React from "react";
import "./Podcast.css";
function Podcast() {
    return (
        <>
            <div className="podcast_heading">
                <h1>READ . WATCH . LISTEN</h1>
                <div className="podcast_quote"><p>Get started now with Insights and Strategies from Toppers</p></div>
            </div>
            <div className="card_podcast">
                <div className="pod_img">
                    <img src="./images/blog.svg" alt="blog_img"></img>
                    <div className="content_name"><p>BLOGS</p></div>
                </div>
                <div className="pod_img">
                    <img src="./images/video.svg" alt="video_img"></img>
                    <div className="content_name"><p>VIDEOS</p></div>
                </div>
                <div className="pod_img">
                    <img src="./images/podcast.svg" alt="podcasts_img"></img>
                    <div className="content_name"><p>PODCASTS</p></div>
                </div>
            </div>
        </>
    )
}
export default Podcast;